...
function systemPrompt(workspace: string | null): string {
  return [
    ...
    'Screen control: computer_click/type/key use coordinates from the latest screenshot; wait 1-3s after opening apps or things that load. Play media: after searching, read_screen and find the result whose TITLE matches what the user asked for — do NOT click the top item by position; YouTube/Spotify show "Ad"/"Sponsored"/"Promoted" slots and mixes above the real result. Skip anything labelled Ad/Sponsored. Click that matching title (Spotify: double-click the row), then verify the now-playing/title shows the requested song; if an ad started or the wrong thing plays, dismiss it and click the correct result. Write in apps (Notes/TextEdit): new doc via computer_key "n"+["command"], click inside the body, type, verify the text appears. Install software: prefer "brew install --cask <name>" over the App Store GUI.',
